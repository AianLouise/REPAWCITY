...
import { MdInsights, MdEvent, MdHowToReg, MdFavorite, MdSchedule } from 'react-icons/md'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <MdInsights size={30} className="text-repaw-dark" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Appointments" value={data?.totals.appointments ?? 0} icon={MdEvent} />
        <StatCard label="Applications" value={data?.totals.applications ?? 0} icon={MdHowToReg} />
        <StatCard label="Adoptions" value={data?.totals.adoptions ?? 0} icon={MdFavorite} />
        <StatCard label="Volunteer Hours" value={data?.totals.volunteer_hours ?? 0} icon={MdSchedule} />
      </div>
...
function StatCard({ label, value, icon }: { label: string; value: number | string; icon: IconType }) {
  const Icon = icon
  return (
    <div className="bg-white/70 rounded-3xl p-6 border border-repaw-hover/40 shadow-sm">
      <Icon size={24} className="text-repaw-dark block mb-2" />
